...
class _ProfileDriverScreenState extends State<ProfileDriverScreen> {
  ...
  Future<void> updateDriverProfile() async {
    try {
      ...
      final response = await AuthService.updateProfile(
        ...
        latitude: latitude,
        longitude: longitude,
        licencePlateNo: _vehicleNo.text.trim(),
        vehiclePlateNo: _vehicleNo.text.trim(),
        vehicleType: selectedVehicle ?? "",
        helpAideStatus: helpAide ? "1" : "0",
        aideName: _aideName.text.trim(),
        aideEmail: _aideEmail.text.trim(),
        aidePhone: _aidePhone.text.trim(),
        aidePhoneCode: selectedDialCode,
        profileImage: (profileImage != null && !isProfileFromApi)
            ? File(profileImage!.path)
            : null,
        aideProfile: (aideProfileImage != null && !isAideFromApi)
            ? File(aideProfileImage!.path)
            : null,
        licenceFront: (frontImage != null && !isFrontFromApi)
            ? File(frontImage!.path)
            : null,
        licenceBack:
        (backImage != null && !isBackFromApi) ? File(backImage!.path) : null,
      );
  ...
  Future<void> fetchProfileData() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Aide fields
        _aideName.text = data.aideName ?? "";
        _aideEmail.text = data.aideEmail ?? "";
        _aidePhone.text = data.aidePhone ?? "";
        helpAide = data.helpAideStatus == "1";
        ...
        if (data.licenceImageFront != null && data.licenceImageFront!.isNotEmpty) {
          frontImage = XFile(data.licenceImageFront!);
          isFrontFromApi = true;
        } else {
          frontImage = null;
        ...
        if (data.licenceImageBack != null && data.licenceImageBack!.isNotEmpty) {
          backImage = XFile(data.licenceImageBack!);
          isBackFromApi = true;
        } else {
          backImage = null;
          isBackFromApi = false;
        ...
        if (data.aideProfile != null && data.aideProfile!.isNotEmpty) {
          aideProfileImage = XFile(data.aideProfile!);
          isAideFromApi = true;
        } else {
          aideProfileImage = null;
          isAideFromApi = false;
        }
      });
  ...
  Widget _profileImageContent() {
    // ✅ No image selected / available
    if (profileImage == null) {
      return _profileIconAvatar();
    }
    // ✅ Image from API (network)
    if (isProfileFromApi) {
  ...
  Widget _buildAideImage() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: ClipOval(
          child: _aideImageContent(),
        ),
      ),
    );
  }

  Widget _aideImageContent() {
    // ✅ No image selected / available
                  ...
                  InkWell(
                    ...
                    child: Text.rich(
                      TextSpan(
                        ...
                        children: [
                          TextSpan(text: "Login", style: GoogleFonts.poppins(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600))
                        ],
                      ),
                    ),
                  ),
